...
def test_route_ignores_non_report_workflows():
    state = {
        "metadata": {
            "next_workflows": [
    ...
    # Should skip PROJECT_INITIALIZATION and route on TECHNICAL_ARCHITECTURE
    assert route_after_project_workflow(state) == REPORT_GENERATOR_NODE


def test_route_ends_when_only_non_report_workflows():
    state = {
        "metadata": {
            "next_workflows": ["PROJECT_INITIALIZATION", "CLARIFICATION"],
            "discovery_complete": True,
            "project_action": "CONTINUE_PROJECT",
        }
    }
